...
public class DataStore {
    ...
    private int getLatestTournamentID() {
        try (PreparedStatement ps = DAO.con.prepareStatement(
                "SELECT MAX(ID) FROM tblTournament")) {
    ...
    public List<PlayerRecord> getRankingForRound(int roundNum) {
        ...
        try (PreparedStatement ps = DAO.con.prepareStatement(sqlPoints)) {
        ...
        try (PreparedStatement ps = DAO.con.prepareStatement(sqlMatchPlayers)) {
            ps.setInt(1, latestTournamentID);
